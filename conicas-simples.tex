...
\draw[->,thick] (-.5,0)--(6,0) node[right]{$OX$};
\draw[->,thick] (0,-.5)--(0,6) node[above]{$OY$};
\node at (0,0) {\textbullet};
\node[below left] at (0,0) {$O$};
\node[below left] at (1.5,0) {$x_0$};
\node[below left] at (0,2) {$y_0$};

\draw[->,thick,blue] (-.5,2)--(6,2) node[right]{$OX'$};
\draw[->,thick,blue] (1.5,-.5)--(1.5,6) node[above]{$OY'$};
...
podem podem ser colocadas na forma reduzida através de uma translação de 
eixos.
...
\begin{frame}[label=conicas]{Rotação}
